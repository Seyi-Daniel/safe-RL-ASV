    ...
    while True:
        ...
        if sampling_logs:
            print(
                ...
                f"reason={status} initial_scenario={initial_scenario} acceptance_scenario={acceptance_scenario} "
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Parallel trainer using CrossingObjectivePort reward for replay/learning")
    ...
    p.add_argument("--epsilon-decay-episodes", type=int, default=TrainParams().epsilon_decay_episodes)
    ...
    p.add_argument("--dcpa-threshold", type=float, default=EnvParams().dcpa_risk_threshold)
    p.add_argument("--tcpa-threshold", type=float, default=EnvParams().tcpa_risk_threshold)
    p.add_argument("--sampling-dcpa-threshold", type=float, default=TrainParams().sampling_dcpa_threshold)
    p.add_argument("--sampling-tcpa-threshold", type=float, default=TrainParams().sampling_tcpa_threshold)
    p.add_argument("--sampling-scenario", type=str, default=None, choices=["head_on", "crossing", "overtaking"])
    p.add_argument("--dcpa-sample-max-tries", type=int, default=0)
    p.add_argument("--max-sampling-steps-per-attempt", type=int, default=0)
    p.add_argument("--sampling-screen-max-steps", type=int, default=TrainParams().sampling_screen_max_steps)
    p.add_argument("--sampling-screen-max-seconds", type=float, default=TrainParams().sampling_screen_max_seconds)
    ...
    p.add_argument(
        "--out-dir",
        type=str,
        default="runs_crossing_port",
        help="base output folder; timestamped run subfolders will be created under this path",
    )
    ...
    p.add_argument("--auto-show-risk-sector-overlay", dest="auto_show_risk_sector_overlay", action="store_true")
    p.add_argument("--no-auto-show-risk-sector-overlay", dest="auto_show_risk_sector_overlay", action="store_false")
    p.set_defaults(auto_show_risk_sector_overlay=EnvParams().auto_show_risk_sector_overlay)
    p.add_argument("--eval-only", action="store_true")
    p.add_argument("--eval-episodes", type=int, default=30)
    p.add_argument("--eval-scenario", type=str, default="all", choices=["head_on", "crossing", "overtaking", "all"])
    p.add_argument("--eval-max-tries-per-episode", type=int, default=200)
    p.add_argument("--eval-checkpoint", type=str, default="")
    return p.parse_args()
...
def _select_runtime_device(cuda_index: int | None) -> torch.device:
    ...
    if selected_index < 0 or selected_index >= device_count:
        raise ValueError(f"Invalid CUDA device index {selected_index}. Available: 0..{device_count - 1}")
...
def _resolve_eval_checkpoint_path(eval_checkpoint: str, out_dir: Path) -> Path:
    ...
    raise FileNotFoundError(
        "No evaluation checkpoint found. Provide --eval-checkpoint, or ensure policy_latest.pt exists "
        "in --out-dir or its newest timestamped run subfolder."
    )
...
def _run_eval_only(
    ...
    for scenario_idx, target_scenario in enumerate(scenarios):
        ...
        print(f"\n[eval-crossing-port] scenario={target_scenario}")
        for ep in range(eval_episodes):
            ...
            if matched_seed is None:
                print(f"[eval] scenario={target_scenario} episode={ep + 1:03d} skipped (no scenario match)")
                continue
            ...
            accepted_episodes += 1
            total_env_return += env_return
            total_port_return += port_return
            total_steps += int(env.step_idx)

        if accepted_episodes == 0:
            print(f"[eval][{target_scenario}] no accepted episodes")
            continue
        print(f"[eval][{target_scenario}] accepted_episode_count={accepted_episodes}")
        print(f"[eval][{target_scenario}] average_env_return={total_env_return / accepted_episodes:.3f}")
        print(f"[eval][{target_scenario}] average_port_return={total_port_return / accepted_episodes:.3f}")
        print(f"[eval][{target_scenario}] average_episode_length={total_steps / accepted_episodes:.2f}")
...
def main() -> None:
    ...
    if args.eval_only:
        _run_eval_only(args, env, agent, reward_adapter)
        env.close()
        sample_env.close()
        return
    ...
    for ep in range(1, train_hp.episodes + 1):
        ...
        accepted_seed, accepted_attempt, _, _, accepted_sample_steps, accepted_scenario = _find_accepted_seed(
            sample_env,
            episode_index=ep,
            base_seed=int(args.seed),
            max_tries=max_tries,
        ...
        if accepted_seed is None:
            print(f"ep={ep:04d} skipped: no accepted seed")
            continue
        ...
        prev_v1_latched = 0
        prev_v2_latched = 0
        ep_replay_insertions = 0
        ep_role_counts = {"vessel1": 0, "vessel2": 0, "none": 0}
        info: dict[str, object] = {}
        ...
        while not done:
            ...
            adapted = reward_adapter.compute_reward(info, action_by_vessel=action_by_vessel, env=env)
            role_key = adapted.vessel_id if adapted.vessel_id in {"vessel1", "vessel2"} else "none"
            ep_role_counts[role_key] += 1
            ...
            # Replay insertion policy: only RL-controlled + designated give-way vessel receives crossing port reward.
            if adapted.replay_eligible and adapted.vessel_id in obs_by_vessel:
                vessel_id = adapted.vessel_id
                vessel_obs = obs_by_vessel[vessel_id]
                vessel_next_obs = env.get_obs_for_vessel(vessel_id)
                vessel_action = action_by_vessel[vessel_id]
                replay.push(vessel_obs, vessel_action, float(adapted.reward), vessel_next_obs, done)
                ep_replay_insertions += 1
        ...
        history.append(
            {
                ...
                "starboard_compliance_rate": float(ep_starboard_compliant / ep_starboard_opportunities)
                if ep_starboard_opportunities
                else 0.0,
                "sample_attempt": int(accepted_attempt),
                "sample_steps": int(accepted_sample_steps),
                "sample_scenario": str(accepted_scenario),
                "sampling_scenario_filter": str(sampling_scenario or "all"),
                "port_reward_vessel_role_count": ep_role_counts,
                "port_replay_entries_inserted": int(ep_replay_insertions),
                "port_reward_replay_used_steps": int(ep_replay_insertions),
            }
        )
        agent.decay_epsilon()

        print(
            f"ep={ep:04d} env_return={env_return:8.3f} port_return={port_return:8.3f} steps={env.step_idx:4d} "
            f"epsilon={eps_now:0.3f} actor_loss={mean_actor_loss:0.4f} critic_loss={mean_critic_loss:0.4f} "
            f"replay={len(replay)} replay_inserted={ep_replay_insertions} collision={int(info.get('collision', 0))} "
            ...
            f"port_reward_replay_used_steps={ep_replay_insertions}"
        )
        ...
        if ep % train_hp.save_every == 0 or ep == train_hp.episodes:
            checkpoint_payload = {
                ...
                "action_dim": ACTION_DIM,
                "algo": "ddpg_crossing_port",
                "train_args": vars(args),
            }
            torch.save(checkpoint_payload, out_dir / f"policy_{ep}.pt")
